...
function getSqliteDbPath() {
  ...
  return path.join(dbDir, 'nutritech.sqlite');
}
...
async function initializeDatabase() {
  ...
  if (process.env.DATABASE_URL && Pool) {
    ...
    connection = { kind: 'postgres', pool };
  } else {
    const db = new sqlite3.Database(getSqliteDbPath(), (err) => {
      if (err) {
        throw err;
      }
    });
    connection = { kind: 'sqlite', db };
  }
...
async function dbRun(sql, params = []) {
  ...
  return new Promise((resolve, reject) => {
    connection.db.run(sql, params, function (err) {
      ...
      resolve({ insertId: this.lastID, rowCount: this.changes });
    });
  });
}
